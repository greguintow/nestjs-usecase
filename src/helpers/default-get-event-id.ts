 ...
 * @returns
 */
export const defaultGetEventId = <EventBase extends IEvent = IEvent>(event: EventBase): string => {
  const { constructor } = Object.getPrototypeOf(event)
  return Reflect.getMetadata(EVENT_METADATA, constructor)?.id ?? null
}

export const defaultReflectEventId = <EventBase extends Type<IEvent> = Type<IEvent>>(
  event: EventBase
): string => {
